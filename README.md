...
The idea in one sentence: **you own what it is for and whether it is right, Claude proposes the
plan and builds it, and nothing counts as done until you have both looked at a screenshot of it
working.**
...
| `skills/turn-critique/` | The most important skill: verify work with real turns and read the evidence |
...
- **A turn** is one real use of your app, the way a user would do it, ending in a screenshot of
  what the user saw (or, for a tool with no screen, what it printed).
- **A critique** is looking at that screenshot as a user and saying what is wrong.
- **A session** is one conversation with Claude Code, from `claude` to `/exit`. Its memory fills up
  and gets muddled. The project folder, with its docs, git history and screenshots, is the memory
  that lasts, so a fresh session can pick up where the last one stopped.
